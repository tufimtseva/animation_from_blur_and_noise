...
class BAistPP(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inp_tensors = [torch.from_numpy(img).float() for img in tensor['inp']]

        if self.noisy:
            inp_tensors = [self.add_noise(img, self.sigma) for img in inp_tensors]

        tensor['inp'] = torch.stack(inp_tensors, dim=0).permute(0, 3, 1, 2)
...
class GenBlur(Dataset):
    def __init__(self, set_type, root_dir, suffix, num_gts, num_fut, num_past, video_list, aug_args,
                 use_trend=False, temporal_step=1, trend_type='trend+', check_trend=False,
                 use_flow=False, flow_type='flow', **kwargs):

        self.noisy = kwargs.get('noisy', False)
        self.sigma = kwargs.get('noise_level', 0)
